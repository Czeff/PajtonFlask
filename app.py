...
MAX_IMAGE_SIZE = 600  # Zwiększono dla lepszej jakości
...
def create_smooth_svg_path(contour):
    """Tworzy wygładzoną ścieżkę SVG z konturu"""
    if len(contour) < 3:
        return None
    ...
    try:
        ...
        for i in range(1, len(contour)):
            ...
            if i % 3 == 0 and i < len(contour) - 2:
                # Użyj krzywej kubicznej co trzeci punkt
...
def generate_professional_svg(svg_paths, width, height):
    """Generuje profesjonalny SVG z ścieżkami haftu"""
...
<svg width="{width}" height="{height}" viewBox="0 0 {width} {height}" 
  ...
  <g inkscape:label="Embroidery Patterns" inkscape:groupmode="layer" class="embroidery-quality">'''
    ...
    # Dodaj ścieżki z parametrami haftu
    for i, (color, path_data) in enumerate(svg_paths):
        color_hex = f"#{color[0]:02x}{color[1]:02x}{color[2]:02x}"
        color_rgb = f"rgb({color[0]},{color[1]},{color[2]})"
        
        # Parametry haftu w zależności od jasności koloru
        brightness = sum(color) / 3
        if brightness < 85:
            # Ciemne kolory - gęste wypełnienie
            row_spacing = "0.3"
            angle = "45"
            stitch_length = "2.5"
        elif brightness > 170:
            # Jasne kolory - rzadsze wypełnienie
            row_spacing = "0.5"
            angle = "135"
            stitch_length = "3.0"
        else:
            # Średnie kolory
            row_spacing = "0.4"
            angle = str(45 + (i * 30) % 180)  # Różne kąty dla różnych kolorów
            stitch_length = "2.8"
...
def create_realistic_preview(svg_path, preview_path, original_image, size=(400, 400)):
    """Tworzy realistyczny podgląd z wektoryzowanego SVG"""
    try:
        ...
        try:
            # Spróbuj załadować domyślną czcionkę
